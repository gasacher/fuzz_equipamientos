...
export function InstrumentForm({
  ...
  const [traceFields, setTraceFields] = useState<TraceabilityFieldValues>(() => {
    if (initial?.status) {
      return {
        ...
        receiptFile: null,
        receiptUrl: initial.receiptPath ? receiptDownloadPath(initial.id) : null,
        ...traceabilityFieldsFromPublished({
          ig: initial.ig,
          fb: initial.fb,
          ml: initial.ml,
    ...
    return defaultTraceabilityFields();
  });
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (showTraceability && !traceFields.location.trim()) {
      setError("Indicá la ubicación del producto");
      setLoading(false);
      return;
    }

    if (showTraceability && traceFields.receiptError) {
    ...
    const published = showTraceability
      ? publishedNetworksToDb(traceFields.status, publishedNetworksFromFields(traceFields))
      : { ig: String(fd.get("ig") || "") || null, fb: false, ml: false };
    ...
    const body = {
      ...
      ...(showTraceability
        ? {
            status: traceFields.status,
            location: traceFields.location,
            buyer: traceFields.status === "vendido" ? traceFields.buyer : null,
            receiptName: traceFields.status === "vendido" ? traceFields.receiptName : null,
            traceNote: traceFields.note || null,
          }
        : {}),
    };
    ...
    const instrumentId = initial?.id ?? data.id;

    if (
      showTraceability &&
      traceFields.status === "vendido" &&
      traceFields.receiptFile
    ) {
      const uploadFd = new FormData();
      uploadFd.append("file", traceFields.receiptFile);
      const up = await fetch(`/api/instruments/${instrumentId}/receipt`, {
        method: "POST",
        body: uploadFd,
      });
      if (!up.ok) {
        ...
        setError(
          (upData.error as string) ??
            "Producto guardado, pero no se pudo adjuntar el recibo.",
        );
        return;
      }
    }
    ...
    router.push(
      initial ? `/admin/inventario/${instrumentId}` : `/admin/inventario/${instrumentId}`,
    );
    router.refresh();
  }
    ...
    <form onSubmit={onSubmit} className="mx-auto max-w-2xl space-y-6">
      {showTraceability && (
        <div className="fuzz-card space-y-4 p-6">
          <InstrumentTraceabilityFields
            {...traceFields}
            heading={initial ? "Estado y ubicación" : "Estado al ingreso"}
      ...
      <div className="fuzz-card space-y-4 p-6">
        <h2 className="fuzz-title text-xl">
          {initial ? "Datos del producto" : "Nuevo instrumento"}
        </h2>
