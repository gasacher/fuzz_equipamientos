...
export function WhatsAppButton({ titulo, precio, className = "", size = "lg" }: Props) {
  const href = buildWhatsAppConsultUrl(titulo, precio);
  const sizeClass = size === "lg" ? "btn-fuzz gap-2 px-6 py-3 text-base" : "btn-fuzz gap-1.5 px-3 py-2 text-xs";
...
export function WhatsAppFloating() {
  const href = buildWhatsAppConsultUrl("un instrumento del catálogo");
  return (
    <a
      href={href}
      target="_blank"
